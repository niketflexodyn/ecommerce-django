    ...
    <article className="group card flex flex-col overflow-hidden border-t-2 border-transparent transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-gold-500">
      <Link to={`/product/${product.id}`} className="relative block overflow-hidden">
        ...
        {product.category?.name && (
          <span className="absolute left-3 top-3 rounded-full bg-gold-500/20 px-2.5 py-1 text-xs font-semibold uppercase tracking-wide text-gold-700 backdrop-blur-sm">
            {product.category.name}
          </span>
        )}
      </Link>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 p-1">
                ...
                <button
                  type="button"
                  onClick={() => updateQuantity(product.id, cartItem.quantity + 1)}
                  className="flex size-9 items-center justify-center rounded-lg bg-plum-950 text-lg font-medium text-white transition hover:opacity-90"
                >
                  +
                </button>
              </div>
