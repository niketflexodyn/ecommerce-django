...
export default function Navbar() {
  ...
  const navLinkClass = ({ isActive }) =>
    ...
    } after:absolute after:left-0 after:-bottom-1 after:h-[2px] after:rounded-full after:bg-gold-500 after:transition-all after:duration-200 ${
      isActive ? 'after:w-full' : 'after:w-0 hover:after:w-full'
    }`

  return (
    <header
      className={`font-body sticky top-0 z-50 bg-white transition-[box-shadow] duration-300 ${
        scrolled ? 'shadow-lg shadow-slate-900/5' : ''
      }`}
    >
      {/* Subtle gold accent line */}
      <div className="h-[2px] bg-linear-to-r from-transparent via-gold-500/60 to-transparent" />
      ...
      <nav className="mx-auto flex max-w-7xl items-center gap-6 px-4 py-3 sm:px-6 lg:gap-8 lg:px-8">
        ...
        <Link to="/" className="group/logo flex items-center gap-2 shrink-0">
          <span
            className="flex size-9 items-center justify-center rounded-lg bg-gold-500 transition-transform duration-200 group-hover/logo:scale-110"
          >
          ...
          <span
            className="font-display text-xl font-bold text-plum-950 tracking-tight transition-[letter-spacing] duration-300 group-hover/logo:tracking-wide"
          >
            Luxora
          </span>
        </Link>
      ...
      {link.name}
        ...
        <div className="hidden md:flex md:items-center md:gap-3 shrink-0">
          ...
          <form onSubmit={handleSearchSubmit} className="w-56 lg:w-72">
            <div className="group relative">
              ...
              <input
                ...
                className="w-full rounded-full border border-slate-200 bg-slate-50 pl-9 pr-4 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-gold-500/40 focus:border-gold-600 focus:bg-white transition-all duration-200"
              />
          ...
          <Link
            ...
            className="relative flex size-9 shrink-0 items-center justify-center rounded-full text-slate-500 transition-colors hover:bg-slate-100 hover:text-plum-950"
          >
            <CartIcon className="size-[18px]" />
            {cartCount > 0 && (
              <span
                className="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-gold-500 px-1 text-[10px] font-bold leading-none text-plum-950 ring-2 ring-white"
              >
          ...
          {user ? (
            <Menu as="div" className="relative shrink-0">
              <MenuButton className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-white transition-colors duration-200 bg-plum-950 hover:bg-plum-900">
              ...
              >
                ...
                <div className="border-b border-slate-100 px-4 py-3">
                  ...
                  <span className="mt-1.5 inline-block rounded-full bg-gold-500/20 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-gold-700">
                ...
                {(user.role === 'admin' || user.role === 'super_admin') && (
                  <MenuItem>
                    <Link
                      to="/dashboard"
                      className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-slate-700 transition-colors hover:bg-cream data-[focus]:bg-cream"
                    >
                      <svg className="size-4 text-gold-700" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
            ...
            <Menu as="div" className="relative shrink-0">
              <MenuButton className="inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-semibold text-white transition-colors duration-200 bg-plum-950 hover:bg-plum-900">
                Login
                <ChevronDownIcon className="size-3.5" />
              </MenuButton>
              ...
              >
                <MenuItem>
                  <Link
                    to="/login"
                    className="block rounded-lg px-4 py-3 transition-colors hover:bg-cream data-[focus]:bg-cream"
                  >
                ...
                <MenuItem>
                  <Link
                    ...
                    className="block rounded-lg px-4 py-3 transition-colors hover:bg-cream data-[focus]:bg-cream"
                  >
                    <span className="block text-sm font-semibold text-gold-700">Create an account</span>
...
<div className="md:hidden ml-auto flex items-center gap-1">
  ...
  href={link.to}
  onClick={closeMobile}
  className="block rounded-lg px-3 py-2.5 text-sm font-medium transition-colors text-slate-600 hover:bg-slate-50 hover:text-plum-950"
>
  {link.name}
</a>
))}
            ...
            {user ? (
              <>
                <div className="px-3 py-2">
                  ...
                  <p className="text-xs text-slate-500">{user.email}</p>
                  <span className="mt-1 inline-block rounded-full bg-gold-500/20 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-gold-700">
                    {ROLE_LABELS[user.role] || user.role}
                  </span>
                </div>
                ...
                {(user.role === 'admin' || user.role === 'super_admin') && (
                  <Link
                    to="/dashboard"
                    onClick={closeMobile}
                    className="flex items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm font-medium text-plum-950 hover:bg-cream"
                  >
                    <svg className="size-4 text-gold-700" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" d="m3.75 13.5 10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75Z" />
                    </svg>
              ...
              <div className="space-y-1">
                ...
                <Link
                  to="/register"
                  onClick={closeMobile}
                  className="block rounded-lg px-3 py-2.5 text-sm font-semibold text-gold-700 hover:bg-cream"
                >
                  Create an account
                </Link>
              </div>
